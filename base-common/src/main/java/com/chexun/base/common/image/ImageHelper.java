...
public class ImageHelper {
	...
	private static void saveSubImage(BufferedImage image, Rectangle subImageBounds, File subImageFile, int left, int top, int dropWidth, int dropHeight) throws IOException {
		...
		BufferedImage subImage = new BufferedImage(dropWidth, dropHeight, BufferedImage.TYPE_INT_RGB);
		Graphics g = subImage.getGraphics();

		g.setColor(Color.white);
		g.fillRect(0, 0, dropWidth, dropHeight);
		g.drawImage(image.getSubimage(subImageBounds.x, subImageBounds.y, subImageBounds.width, subImageBounds.height), left, top, null);
		g.dispose();
